...
const Claim_Report = () => {
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`${BASE_URL}JazzReport?${queryParams}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleDetailsClick = (customerMobileNo, claim_No) => {
    console.log(`Navigating to /claim_details/${customerMobileNo}/${claim_No}`);
  ...
  const columns = [
    ...
    { name: "Close Date", selector: (row) => row.approvedDate || "N/A", sortable: true },
    { name: "Processing Date", selector: (row) => (row.status === "Processing" ? row.processingDate : "N/A"), sortable: true },
    {
      name: "Action",
      cell: (row) => (
        <Button
          ...
          <form onSubmit={handleSubmit} className="row mb-3">
            ...
            <div className="col-md-3">
              ...
              <select className="form-control" value={status} onChange={(e) => setStatus(e.target.value)}>
                ...
                {['Intimation', 'Processing', 'Rejected', 'Approved', 'Paid'].map((opt) => (
      ...
      <div className="card m-2">
        ...
        {reportData.length > 0 && (
          <div className="card-body">
            ...
            <CSVLink data={reportData} filename="report.csv" className="btn btn-success w-100 mt-3">
