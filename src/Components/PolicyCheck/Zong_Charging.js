...
const calculateDayDifference = (start, end) => {
  ...
  useEffect(() => {
    const fetchChargingData = async () => {
      ...
      try {
        const url = `${Chargingcheck_URL}api/zong/charging/policy`;
        ...
        console.log("Fetched Charging Data:", json.ResponseData); // <-- Debug
      ...
      <Container fluid className="mt-4">
        <Card className="shadow-sm">
          <Card.Body>
           ...
           {data.length > 0 && (
  <div className="mb-3">
    <h6 className="text-secondary">Total Charge Amount</h6>
    <ul className="mb-0">
      {Object.entries(calculateServiceTotals()).map(([serviceId, total]) => (
        <React.Fragment key={serviceId}>
          ...
          <li>
            <strong>Service ID: {serviceId}</strong>: Rs {total.toFixed(2)}
          </li>
